...
- **Amber zsh extras:** `config/hal.zsh` (symlinked, sourced from `~/.zshrc` after starship init) — `walk` (sets/show/clears the day's Shadow in `~/.amber/shadow`), transient prompt collapsing past prompts to `✶ cmd` (`export TRANSIENT_PROMPT=0` to disable), command-not-found in Ghost's voice, macOS notification for commands ≥30s. (Filename is still `hal.zsh` so the `~/.zshrc` `source` marker stays valid; the HAL-voiced originals are preserved in a commented block at the foot.)
...
- **MOTD:** `config/motd.sh` (symlinked to `~/.config/motd.sh`, sourced from `~/.zshrc`) — the **Ghostwheel** emblem (`config/ghostwheel_amber.ansi`, a static 32×16 braille `.ansi` read via `$(<file)`) beside user@host, OS/chip identity, date/uptime, memory/load/battery, disk/LAN IP; a time-aware greeting (Tir-na Nog'th after dark by `America/Chicago`), a daily-rotating curated Amber texture line, and the Shadow you ride toward (from `walk`). Battery ≤20% (discharging) and disk <50G render red. `export MOTD_EMBLEM=0` reverts to the plain stacked greeting. The retired HAL eye is preserved commented in `motd.sh`; `~/.config/hal9000.sh 19` still renders it full-size.
